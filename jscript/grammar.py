...
class Grammar(object):
    ...
    def WriteResultToFile(self, result, out_dir):
        """Write mutated code."""
        self._sampleNumber = self._sampleNumber + 1        
        outfile = os.path.join(out_dir, 'fuzz-' + str(self._sampleNumber).zfill(5) + '.html')
    ...
    def _generate_samplefiles(self, result, out_dir, place_holder=None, replace_string=None ):
        """Generates a mutated code."""            
        if place_holder != None and replace_string != None :
            result = result.replace(place_holder, replace_string, 1)

        for tag_name in self._functionmap:
            place_holder = '<'+tag_name+'>'
            if place_holder in result:
                tag_values  = self._functionmap[tag_name]
                for tag_value in tag_values:
                    for part in tag_value['parts']:
                        if part['type'] == 'text':
                            tag_value_str = part['text'].strip()                            
                            self._generate_samplefiles(result, out_dir, place_holder, tag_value_str.strip('='))
        if place_holder != None and replace_string != None :
            self.WriteResultToFile(result, out_dir)
    ...
    def _parse_function_map(self, line, helper_lines=False):
        """Parses a rule for generating code."""
        rule = {
            'type': 'function',
            'parts': [],
        ...
        create_tag_name = parsedtag['tagname']
        if create_tag_name in self._functionmap:
            self._functionmap[create_tag_name].append(rule)
        else:
            self._functionmap[create_tag_name] = [rule]
    ...
    def _include_from_string(self, grammar_str):
        ...
        helper_lines = False
        in_function = False
        in_functionmap = False
        in_statementsmap = False
        num_errors = 0
        ...
        for line in lines:
            ...
            if match:
                ...
                if command in self._command_handlers:
                ...
                elif command == 'begin' and params == 'functionmap':
                    in_functionmap = True
                    helper_lines = False
                elif command == 'begin' and params == 'statementsmap':
                    in_statementsmap = True
                    helper_lines = False
                    ...
                    if in_code:
                        in_code = False
                elif command == 'end' and params in ('functionmap', 'statementsmap'):
                    if in_functionmap:
                        in_functionmap = False
                    if in_statementsmap:
                        in_statementsmap = False						
            ...
            try:
                ...
                elif in_statementsmap:
                    self._parse_function_map(cleanline, helper_lines)
                elif in_functionmap:
                    self._parse_function_map(cleanline, helper_lines)
                elif in_code:
    ...
    def _get_variable_setters(self, var_name, var_type):
        ...
        return ret#   Domato - grammar parser and generator
#   --------------------------------------
#
#   Written and maintained by Ivan Fratric <ifratric@google.com>
#
